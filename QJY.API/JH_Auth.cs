...
namespace QJY.API
{
    ...
    public class JH_Auth_BranchB : BaseEFDao<JH_Auth_Branch>
    {
        ...
        {
            string strSql = string.Format("SELECT DeptCode from JH_Auth_Branch where ComId={0}  And  " + SqlHelp.concat("','+TXLQX+','") + "  NOT LIKE '%,{1},%' and TXLQX!='' and TXLQX is NOT NULL and IsHasQX='Y'", userInfo.User.ComId, userInfo.User.BranchCode);
    ...
    public class FT_FolderB : BaseEFDao<FT_Folder>
    {
        ...
        public string isHasManage(string FloderID, string strUserName)
        {
            ...
            if (!string.IsNullOrEmpty(Model.Remark))
            {
                ...
                DataTable dt = new FT_FolderB().GetDTByCommand("SELECT ID FROM FT_Folder WHERE " + SqlHelp.concat("','+UploadaAuthUsers+','") + " like '%," + strUserName + ",%'  AND ID IN ('" + Model.Remark.ToFormatLike('-') + "')");
                if (dt.Rows.Count > 0)
                {
                    strReturn = "Y";
                }
    ...
    public class FT_FileB : BaseEFDao<FT_File>
    {
        ...
        /// <param name="FolderID"></param>
        /// <returns></returns>
        public FT_File GetSameFile(string strFileName, string strkzname, int FolderID, int ComId)
        {
            int[] folders = { 1, 2, 3 };
            if (!folders.Contains(FolderID))
            {
                return new FT_FileB().GetEntities(d => d.ComId == ComId && d.Name == strFileName && d.FileExtendName == strkzname && d.FolderID == FolderID).FirstOrDefault();
            }
            return null;
        ...
        public void ADDFILE(Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
            ...
            try
            {
                ...
                foreach (var item in Files)
                {
                    ...
                    FT_File File = new FT_FileB().GetSameFile(filename, item["filename"].ToString().Substring(index + 1).ToLower(),int.Parse(P2), UserInfo.User.ComId.Value);
