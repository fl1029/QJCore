...
namespace QJY.API
{
    public class DXGLManage 
    {
        ...
        public void GETDXGLLIST(JObject context, Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
            ...
            if (status != "")
            {
                strWhere += string.Format(" And  SendTime {0} '{1}'", status=="0"?"<":">",DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            }           
            int page = 0;
            int pagecount = 8;
            ...
            page = page == 0 ? 1 : page;
            int total = 0;
            DataTable dt = new SZHL_DXGLB().GetDataPager(" SZHL_DXGL ", "ID,dxContent,dxnums,SendTime,CRUser,CRDate,case when SendTime<='"+ DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' then '已发送' else '待发送' end as status", pagecount, page, " CRDate desc", strWhere, ref total);
            msg.Result = dt;
            msg.Result1 = total;
        }

        //获取短信内容
        public void GETDXGLMODEL(JObject context, Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
            int Id = int.Parse(P1);
        ...
        /// <param name="P2"></param>
        /// <param name="UserInfo"></param>
        public void DELDXGL(JObject context, Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
            try
            {
                int ID = int.Parse(P1);
        ...
        public void DXGLCHECK(JObject context, Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
        ...
        public void GETDXQTY(JObject context, Msg_Result msg, string P1, string P2, JH_Auth_UserB.UserInfo UserInfo)
        {
