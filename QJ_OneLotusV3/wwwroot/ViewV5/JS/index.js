var model = avalon.define({
        ...
        if (model.isiframe == 'Y') {
            ...
            if (pagecode) {
                ...
                for (var i = 0; i < model.FunData.length; i++) {
                    if (model.FunData[i].PageCode.indexOf(pagecode) > -1 && model.initobj == model.FunData[i].ExtData) {
    ...
    GetUserData: function () {
        ...
        $.getJSON('/api/Bll/ExeAction?Action=XXFB_GETXXFBLIST', { "type": "1" }, function (resultData) {
            if (resultData.ErrorMsg == "") {
    ...
    SETKJFS: function (item, event) {
        ...
        $.getJSON('/api/Auth/ExeAction?Action=SETAPPINDEX', { "P1": item.ModelCode, "Status": item.issel ? "N" : "Y", type: "PCKJFS", name: item.ModelName }, function (result) {
            if (result.ErrorMsg == "") {
    ...
    SaveYY: function (item) {
        ...
        if (strContent) {
            $.getJSON("/api/Auth/ExeAction?Action=SETAPPINDEX", { P1: strContent, type: "PCINDEX" }, function (jsonresult) {
    ...
    GetCustomData: function (P1, callback) {//P1:类型，callback:回调函数
        $.getJSON('/api/Auth/ExeAction?Action=GETUSERGROUP', { P1: P1 }, function (resultData) {
            if (resultData.ErrorMsg == "") {
                if (callback) {
                    return callback.call(this, resultData.Result);
                }
    ...
    GetExtColumns: function (str) {  //获取扩展字段
        ...
        if (model.SelModel && model.rdm == str) {
            $.getJSON('/api/Auth/ExeAction?Action=GETEXTENDFIELD', { P1: model.SelModel.ModelCode }, function (resultData) {
                if (resultData.ErrorMsg == "") {
