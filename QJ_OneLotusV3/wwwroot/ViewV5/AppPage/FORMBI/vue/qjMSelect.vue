<template>
    <el-col :sm="24" :md="pzoption.mdwidth">
        ...
        <el-form-item :label="pzoption.title" :prop="'wigetitems.' + index + '.value'" :rules="childpro.rules">
            ...
            <el-select v-model="childpro.selval" style="width:100%" :disabled="childpro.disabled" multiple filterable clearable>
        ...
        <el-dialog title="组件属性" :visible.sync="dialogInputVisible" width="30%">
            <el-tabs type="border-card">
                <el-tab-pane label="个性配置" style="min-height: 420px;">
...
<script>
    module.exports = {
        ...
        methods: {
            ...
            VailSql: function () {
                var chi = this;
                $.getJSON("/api/Bll/ExeAction?Action=FORMBI_GETSQLDATA", {  P1: chi.childpro.datasql }, function (result) {
                    if (!result.ErrorMsg) {
                        app.$notify({
                            title: '成功',
                            message: '加载数据成功',
            ...
            handleChange: function (arrval) {
                var app = this;
                $.getJSON("/api/Bll/ExeAction?Action=FORMBI_GETBDTJDATA", {  P1: arrval[0], pdfields: arrval[1] }, function (result) {
                    if (!result.ErrorMsg) {
                        var temparr = [];
                        _.forEach(result.Result, function (obj) {
        ...
        mounted: function () {
            ...
            pro.$nextTick(function () {
                ...
                //获取关联字段可用的表单数据
                $.getJSON("/api/Bll/ExeAction?Action=FORMBI_GETFORMFILEDS", {  }, function (result) {
                    if (!result.ErrorMsg) {
                        pro.glformdata = result.Result;
                    }
                })
                if (pro.pzoption.childpro.datatype) {
                    pro.childpro = pro.pzoption.childpro;
                    if (pro.childpro.datatype == '1') {
                        pro.VailSql();
                    }
                } else {
                    pro.senddata();
                ...
                if (pro.pzoption.value) {
                    pro.childpro.selval = pro.pzoption.value.split(',');
                }
        ...
        watch: {
            ...
            "childpro.selval": { //深度监听，可监听到对象、数组的变化
                handler(newV, oldV) {
                    if (newV) {
