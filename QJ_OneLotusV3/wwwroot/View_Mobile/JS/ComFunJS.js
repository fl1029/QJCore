...
var ComFunJS = {
        ...
        ComFunJS.loadJs(ComFunJS.getfileapi() + "/Web/qj_upload.js", function () {
            ...
            var obj = {
                ...
                closeupwin: function (fileData) {
                    var fjids = "";
                    $.getJSON('/api/Bll/ExeAction?Action=QYWD_ADDFILE', { "P1": fileData, "P2": 3 }, function (result) {
                        if (result.ErrorMsg == "") {
    ...
    viewimg: function (tpdata) {
        ...
        $(tpdata).each(function (index, ele) {
            var html = '';
            if (ComFunJS.xstp(ele.FileExtendName)) {
                html = '<li itemid="' + ele.ID + '" class="weui_uploader_file mall_pcp" onclick="ComFunJS.viewbigimg(this,2)" style="background-image:url(' + ComFunJS.getfile(ele.ID) + ')" src="' + ComFunJS.getfile(ele.ID) + '"></li>';
            }
            else {
    ...
    UploadLoad: function (str) {
        if (document.getElementById("file").files.length > 0) {
            ...
            $.ajax({
                url: "/api/Auth/ExeAction?Action=UPLOADFILE&P1=" + str + "&r=" + Math.random(),
                type: "POST",
                data: formData,
                /**
        ...
        getJSON: function (url, data, success, opt) {
            ...
            $.ajax({
                ...
                beforeSend: function (XHR) {
                    XHR.setRequestHeader("Authorization", "Bearer " + ComFunJS.getCookie("szhlcode"));
                },
